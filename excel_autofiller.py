...
class ExcelAutofiller:
    ...
    def open_workbook(self):
        ...
        try:
            ...
            if self.week_day not in self.wb.sheetnames:
                ...
                print(f"     Available sheets: {', '.join(self.wb.sheetnames)}")
                return False
    ...
    def find_row(self):
        ...
        for r in range(self.start_row, self.ws.max_row + 1):
            ...
            if cell_value and cell_value.strip().lower() == self.location.lower():
                self.row = r
                print(f"  ✓ Found location '{self.location}' at row {self.row}")
                return True
        
        # Location not found
        print(f"  ❌ Location '{self.location}' not found in worksheet '{self.week_day}'")
    ...
    def checking_tenders(self):
        ...
        if over_col is None:
            print("  ⚠️  'over' column not defined in CHECKING_COL_MAP - skipping validation")
            return True
        ...
        if over_value is not None:
            try:
                ...
                if over_amount != 0:
                    print(f"  ⚠️  Tender discrepancy detected: ${over_amount:.2f}")
                    return False
            except (ValueError, TypeError):
                print(f"  ⚠️  Invalid value in over/short column: {over_value}")
                return False
    ...
    def filling(self, parser):
        ...
        try:
            ...
            if total_sales_col:
                self.ws.cell(self.row, total_sales_col).value = parser.get("total_sales")
            ...
            if unmatched_tenders:
                print(f"  ⚠️  Unmatched tenders (not filled): {', '.join(unmatched_tenders)}")
    ...
    def save(self):
        ...
        try:
            ...
            if is_correct:
                print(f"  ✅ {self.location} validated - all tenders balance correctly")
            else:
                print(f"  ⚠️  {self.location} validation warning - check over/short column")
