            ...
            while index < len(self.lines) and self.lines[index][0]:
                ...
                if value is not None:
                    if 'Net Tax' in key:
                        ...
                        print(f"  ✓ Total Sales: ${self.data['total_sales']:.2f}")
                        found_total = True
        ...
        try:
            ...
            while index < len(self.lines) and self.lines[index][0]:
                ...
                if tender_name in TAVLO_TENDERS:
                    ...
                    self.data["tenders"][casheet_tender_name] += float(tender_value)
                else:
            ...
            if unrecognized_tenders:
                print(f"  ⚠️  Unrecognized tenders in {section_name}: {', '.join(unrecognized_tenders)}")
        ...
        try:
            ...
            if index > self.index["coupons"] + 1:
                ...
                if coupon_value is not None and "Total" in coupon_key:
                    ...
                    self.data["tenders"][converted_coupons_name] = float(coupon_value)
                    print(f"  ✓ Coupons: ${self.data['tenders'][converted_coupons_name]:.2f}")
                    return True
